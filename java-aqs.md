...
不同的自定义同步器争用共享资源的方式也不同。自定义同步器在实现时只需要实现共享资源state的获取与释放方式即可，至于具体线程等待队列的维护（如获取资源失败入队/唤醒出队等），AQS已经在低层实现好了。自定义同步器实现时主要实现以下几种方法：
...
from http://www.cnblogs.com/waterystone/p/4920797.html
